...
let handleCounterData = data => {
  ...
  Js.Dict.set(dict, "stocks", data);
  // let json: Js.Json.t = Js.Json.object_(dict);
  /* turn it into a proper record */
  let counters: array(Stock.t) =
    Json.Decode.decodeArray(Stock.Decode.stockItem(json));
  Js.log2("counters:", counters->Obj.magic->Belt.List.toArray);
  counters;
};
